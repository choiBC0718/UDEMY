...
{
	return (GetCapacity() <= InventoryMap.Num());
}
...
void UInventoryComponent::GrantItem(const UPA_ShopItem* NewItem)
{
	...
	{
		if (CheckItemCombination(NewItem))
			return;
...
bool UInventoryComponent::CheckItemCombination(const UPA_ShopItem* NewItem)
{
	if (!GetOwner()->HasAuthority())
		return false;
...
void UInventoryComponent::Server_Purchase_Implementation(const UPA_ShopItem* ItemToPurchase)
{
	...
	if (CheckItemCombination(ItemToPurchase))
	{
		OwnerASC -> ApplyModToAttribute(UCHeroAttributeSet::GetGoldAttribute(), EGameplayModOp::Additive, -ItemToPurchase->GetPrice());
	}
}
...
void UInventoryComponent::Client_ItemAdded_Implementation(FInventoryItemHandle AssignedHandle, const UPA_ShopItem* Item, FGameplayAbilitySpecHandle GrantedSpecHandle)
{
	if (GetOwner()->HasAuthority())
		return;

	UInventoryItem* InventoryItem = NewObject<UInventoryItem>();
	InventoryItem->InitItem(AssignedHandle, Item,OwnerASC);
