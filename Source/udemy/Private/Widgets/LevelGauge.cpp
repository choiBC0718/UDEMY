...
void ULevelGauge::NativeConstruct()
{
	...
	APawn* OwerPawn = GetOwningPlayerPawn();
	if (!OwerPawn)	return;

	UAbilitySystemComponent* OwnerAbilitySystemComponent = UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(OwerPawn);
...
void ULevelGauge::UpdateGauge(const FOnAttributeChangeData& Data)
{
	...
	LevelText -> SetText(FText::AsNumber(CurrentLevel, &FormattingOptions));
